...
import {isShowSuccessModal} from "@store/actions";
import {State} from "@tsTypes/state";
import {connect, ConnectedProps} from "react-redux";

const mapStateToProps = ({isShowSuccessModal}: State) => ({isShowSuccessModal});
const connector = connect(mapStateToProps);

type PropsFromRedux = ConnectedProps<typeof connector>

class Success extends Component<PropsFromRedux> {
    componentDidMount() {
    ...
    handleCloseModal(e: MouseEvent) {
        e.preventDefault()
        document.body.style.overflow = "visible";
        this.props.dispatch(isShowSuccessModal(false));
    }
            ...
            <section className="modal">
                <form className="modal_form">
                    <fieldset className="modal_form-fieldset">
                        ...
                        <legend className="modal_form-caption">Изменения успешно сохранены</legend>
                        <button onClick={((e) => this.handleCloseModal(e))} className="modal_form-confirm-button">Ок
                        </button>
                    </fieldset>
                </form>
            </section>
